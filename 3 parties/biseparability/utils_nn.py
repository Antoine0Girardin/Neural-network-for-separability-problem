...
def integral(r_ABC, r_BAC, r_CAB, p1, p2, p3):
    """compute the integral with multiple matrices"""
    s=0
    for i in range(r_ABC.shape[0]):
        s+= p1[i]* (p2[i]*r_ABC[i] + p3[i]*r_BAC[i] + (1-p2[i]-p3[i])*r_CAB[i])
    return s
...
def strategy(y_pred):
    ...
    b=y_pred[:, l_a:l_a*2]
    c=y_pred[:, l_a*2:l_a*3]
    
    bc=y_pred[:, l_a*3:l_a*3+l_b]
    ac=y_pred[:, l_a*3+l_b:l_a*3+l_b*2]
    ab=y_pred[:, l_a*3+l_b*2:l_a*3+l_b*3]
    
    p1=y_pred[:, l_a*3+l_b*3:l_a*3+l_b*3+1] 
    p2=y_pred[:, l_a*3+l_b*3+1:l_a*3+l_b*3+2]
    p3=y_pred[:, l_a*3+l_b*3+2:l_a*3+l_b*3+3]  
    
    p1=tf.cast(p1, 'complex64')
    p2=tf.cast(p2, 'complex64')
    p3=tf.cast(p3, 'complex64')
                  
    weight1=p1/tf.math.reduce_sum(p1)  #weight1 sum to one
    weight2=p2
    weight3=p3*(1-p2)                   #weight2 < 1-p2
    
    for i in range(cf.pnn.batch_size):
        if i == 0:
            r_a = densitynK(tf.reshape(tf.slice(a, (0,0), (1,(2*n_a-1))),((2*n_a-1),)), n_a)
            r_b = densitynK(tf.reshape(tf.slice(b, (0,0), (1,(2*n_a-1))),((2*n_a-1),)), n_a)
            r_c = densitynK(tf.reshape(tf.slice(c, (0,0), (1,(2*n_a-1))),((2*n_a-1),)), n_a)
            
            r_bc = densitynK(tf.reshape(tf.slice(bc, (0,0), (1,(2*n_b-1))),((2*n_b-1),)), n_b)
            r_ac = densitynK(tf.reshape(tf.slice(ac, (0,0), (1,(2*n_b-1))),((2*n_b-1),)), n_b)
            r_ab = densitynK(tf.reshape(tf.slice(ab, (0,0), (1,(2*n_b-1))),((2*n_b-1),)), n_b)

        else:
            r_a = tf.concat([r_a, densitynK(tf.reshape(tf.slice(a, (i,0), (1,(2*n_a-1))), ((2*n_a-1),)), n_a)], axis=0)
            r_b = tf.concat([r_b, densitynK(tf.reshape(tf.slice(b, (i,0), (1,(2*n_a-1))), ((2*n_a-1),)), n_a)], axis=0)
            r_c = tf.concat([r_c, densitynK(tf.reshape(tf.slice(c, (i,0), (1,(2*n_a-1))), ((2*n_a-1),)), n_a)], axis=0)
            
            r_bc = tf.concat([r_bc, densitynK(tf.reshape(tf.slice(bc, (i,0), (1,(2*n_b-1))), ((2*n_b-1),)), n_b)], axis=0)
            r_ac = tf.concat([r_ac, densitynK(tf.reshape(tf.slice(ac, (i,0), (1,(2*n_b-1))), ((2*n_b-1),)), n_b)], axis=0)
            r_ab = tf.concat([r_ab, densitynK(tf.reshape(tf.slice(ab, (i,0), (1,(2*n_b-1))), ((2*n_b-1),)), n_b)], axis=0)

    r_a=tf.reshape(r_a, (cf.pnn.batch_size,n_a,n_a))
    r_b=tf.reshape(r_b, (cf.pnn.batch_size,n_a,n_a))
    r_c=tf.reshape(r_c, (cf.pnn.batch_size,n_a,n_a))
    
    r_bc=tf.reshape(r_bc, (cf.pnn.batch_size,n_b,n_b))
    r_ac=tf.reshape(r_ac, (cf.pnn.batch_size,n_b,n_b))
    r_ab=tf.reshape(r_ab, (cf.pnn.batch_size,n_b,n_b))
    
    r_ABC = kron_prod(r_a, r_bc)
    
    r_BAC = kron_prod(r_b, r_ac)
    
    for i in range(r_BAC.shape[0]):
        if i==0:
            r_BAC2 = reorder(r_BAC[i])
            r_BAC2 = tf.reshape(r_BAC2, (1, n_a*n_b, n_a*n_b))
        else:
            r_BAC2 = tf.concat([r_BAC2, tf.reshape(reorder(r_BAC[i]), (1, n_a*n_b, n_a*n_b))], 0)
    
    r_CAB = kron_prod(r_ab, r_c)
    return r_ABC, r_BAC, r_CAB
...
def customLoss_matrix(y_pred):          ##y_pred is what the nn gives out size (batch_size, output size =6 (3 for each matrix))
    ...
    return integral(r_ABC, r_BAC2, r_CAB, weight1, weight2, weight3)
