...
import qutip as q
from targets import GHZ, W_state
...
class Config:
    ...
    def __init__(self):
        ...
        self.inputsize=10
        self.a_outputsize = (2*self.dim_a-1)*3+(2*self.dim_b-1)*3+3  ## 2n-1 for each pure states, +3 for p(lambda) 
        ...
        self.weight_init_scaling = 10. # default is 1. Set to larger values to get more variance in initial weights.
    ...
    def change_dmtargetGHZ(self, a):
        self.dmtarget = a*GHZ(self.dim_a, self.dim_b) + (1-a)*tf.cast(tf.eye(8)/8, 'complex64')
